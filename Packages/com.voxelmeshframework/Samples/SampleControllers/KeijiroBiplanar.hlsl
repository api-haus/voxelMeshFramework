...
void Biplanar_float(Texture2D tex, SamplerState samp, float3 wpos, float3 wnrm,
                    out float4 output) {
  ...
  // Project + fetch
  float4 x = SAMPLE_TEXTURE2D_GRAD(tex, samp, float2(p[ma.y], p[ma.z]),
                                   float2(dpdx[ma.y], dpdx[ma.z]),
                                   float2(dpdy[ma.y], dpdy[ma.z]));

  float4 y = SAMPLE_TEXTURE2D_GRAD(tex, samp, float2(p[me.y], p[me.z]),
...
void BiplanarNormal_float(Texture2D tex, SamplerState samp, float3 wpos,
                          float3 wtan, float3 wbtn, float3 wnrm,
                          out float3 output) {
  // Coordinate derivatives for texturing
  float3 p = wpos;
  float3 n = abs(wnrm);
