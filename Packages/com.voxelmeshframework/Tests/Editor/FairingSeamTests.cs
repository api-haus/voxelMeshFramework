namespace Voxels.Tests.Editor
{
	...
	public class FairingSeamTests
	{
		...
		public void HemisphereIsland_1x1x1_PairedChunks_FairedSurfaceNets_SeamVerticesMatch()
		{
			...
			var a = new NativeVoxelMesh(Unity.Collections.Allocator.TempJob);
			var b = new NativeVoxelMesh(Unity.Collections.Allocator.TempJob);
			try
			{
				...
				var boundsA = Unity.Mathematics.Geometry.MinMaxAABB.CreateFromCenterAndExtents(
					originA + (new float3(VoxelConstants.CHUNK_SIZE * 0.5f) * voxelSize),
					new float3(VoxelConstants.CHUNK_SIZE * voxelSize)
				);
				var boundsB = Unity.Mathematics.Geometry.MinMaxAABB.CreateFromCenterAndExtents(
				...
				// Sync overlap (X axis)
				StampScheduling.ScheduleCopySharedOverlap(a, b, axis: 0).Complete();
		...
		public void HemisphereIsland_2x2x2_FairedSurfaceNets_SeamsAreStable()
		{
			...
			Mesh combined = GridMeshingBuilder.BuildCombinedMesh(options, generator, schedulerFactory);
