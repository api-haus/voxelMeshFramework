namespace Voxels.Core.Hybrid.GameObjectLifecycle
{
	...
	public partial struct EntityInstanceIDLifecycleSystem : ISystem
	{
		...
		{
			using var _ = EntityInstanceIDLifecycleSystem_Update.Auto();
			...
			//TODO: optimise, look at structural changes

			using (EntityInstanceIDLifecycleSystem_MapUpdate.Auto())
			{
				foreach (
			...
			using (EntityInstanceIDLifecycleSystem_Destroy.Auto())
			{
				foreach (
					...
					if (st.map.TryGetValue(evt.ValueRO.gameObjectInstanceID, out var associatedEntity))
					{
						...
						st.map.Remove(evt.ValueRO.gameObjectInstanceID);
					}
			}
		}
	}
