namespace Voxels.Core.ThirdParty.SurfaceNets.Intrinsics
{
	...
	public static unsafe partial class X86F
	{
		...
		public static class Sse4_1
		{
			...
			public static v128 stream_load_si128(void* memAddr)
			{
				return GenericCSharpLoad(memAddr);
			}
			...
			static double RoundDImpl(double d, int roundingMode)
			{
				switch (roundingMode & 7)
				{
					...
					default:
						switch (Mxcsr & MXCSRBits.ROUNDING_CONTROL_MASK)
						{
							case MXCSRBits.ROUND_TO_NEAREST:
								return Math.Round(d);
							case MXCSRBits.ROUND_DOWN:
								return Math.Floor(d);
							case MXCSRBits.ROUND_UP:
