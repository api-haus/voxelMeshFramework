namespace Voxels.Samples.SampleControllers
{
	...
	public enum RaySource
	{
		[InspectorName("Viewport Center")]
		VIEWPORT_CENTER,

		[InspectorName("Pointer Position")]
		POINTER_POSITION,
	}
	...
	public class SampleFirstPersonDiggingCamera : MonoBehaviour
	{
		...
		{
			var ray =
				raySource == RaySource.VIEWPORT_CENTER
					? m_Camera.ViewportPointToRay(Vector3.one * .5f)
					: m_Camera.ScreenPointToRay(Pointer.current?.position.value ?? -Vector2.one);
