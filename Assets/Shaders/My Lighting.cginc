...
void ComputeVertexLightColor(inout InterpolatorsVertex i) {
  ...
  i.vertexLightColor = Shade4PointLights(
      ...
      unity_LightColor[3].rgb, unity_4LightAtten0, i.worldPos.xyz, i.normal);
#endif
}
...
InterpolatorsVertex MyVertexProgram(VertexData v) {
  ...
  float3x3 objectToTangent = float3x3(
      v.tangent.xyz, cross(v.normal, v.tangent.xyz) * v.tangent.w, v.normal);
...
// UNITY_LIGHT_ATTENUATION doesn't fade shadows for us.
  ...
  attenuation = UnityMixRealtimeAndBakedShadows(attenuation, bakedAttenuation,
                                                shadowFade);
#endif
...
void ApplySubtractiveLighting(Interpolators i,
                              inout UnityIndirect indirectLight) {
...
UnityIndirect CreateIndirectLight(Interpolators i, float3 viewDir,
  ...
  if (unity_ProbeVolumeParams.x == 1) {
    indirectLight.diffuse =
        SHEvalLinearL0L1_SampleProbeVolume(float4(i.normal, 1), i.worldPos);
...
float GetParallaxHeight(float2 uv) { return tex2D(_ParallaxMap, uv).g; }
...
#define PARALLAX_RAYMARCHING_STEPS 10
  ...
  for (int i = 1; i < PARALLAX_RAYMARCHING_STEPS && stepHeight > surfaceHeight;
       i++) {
...
FragmentOutput MyFragmentProgram(Interpolators i) {
  ...
  float4 color = UNITY_BRDF_PBS(
      albedo, specularTint, oneMinusReflectivity, surface.smoothness, i.normal,
      viewDir, CreateLight(i), CreateIndirectLight(i, viewDir, surface));
