...
void ScreenSpaceRatio2_float(float4x4 projection, float4 position,
                             ...
                             float screenHeight, float fontScale,
                             out float SSR) {
  float2 pixelSize = position.w;
  pixelSize /= (objectScale *
                mul((float2x2)projection, float2(screenWidth, screenHeight)));
  SSR = rsqrt(dot(pixelSize, pixelSize) * 2) * fontScale;
}

// UV			: Texture coordinate of the source distance field
// texture TextureSize	: Size of the source distance field texture Filter
// : Enable perspective filter (soften)
...
void ComputeSDF_float(float SSR, float SD, float SDR, float isoPerimeter,
  ...
  outAlpha = saturate(
      (d * 2.0 * SSR + 0.5 + isoPerimeter * SDR * SSR + softness * 0.5) /
      (1.0 + softness));
...
void Layer4_float(float4 alpha, float4 color0, float4 color1, float4 color2,
  ...
  color2.rgb *= color2.a;
  color3.rgb *= color3.a;
  outColor = lerp(lerp(lerp(color3, color2, alpha.z), color1, alpha.y), color0,
                  alpha.x);
  outColor.rgb /= outColor.a;
}
