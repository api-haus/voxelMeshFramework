...
struct SingleUVTextureArraySampler {
  ...
  void sampleNormal(in float3 worldNormal, in int chan, TEXTURE2D_ARRAY(t2d),
    ...
    // GPU Gems 3 style world-space contributions with zero along axis normal
    half3 normalX = half3(0.0h, tnorm.yx);
    half3 normalY = half3(tnorm.x, 0.0h, tnorm.y);
    half3 normalZ = half3(tnorm.xy, 0.0h);

    normal = normalize(normalX * triWeights.x + normalY * triWeights.y +
                       normalZ * triWeights.z + worldNormal);
