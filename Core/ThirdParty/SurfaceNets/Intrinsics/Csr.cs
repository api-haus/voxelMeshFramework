namespace Voxels.Core.ThirdParty.SurfaceNets.Intrinsics
{
	...
	[BurstCompile]
#endif
	public static partial class X86F
	{
		/// <summary>
		...
		public enum MXCSRBits
		{
			...
			/// </summary>
			/// <remarks>
			///   When the underflow exception is masked and the flush-to-zero mode is enabled, the processor performs the following
			...
			///   The flush-to-zero mode is not compatible with IEEE Standard 754. The IEEE-mandated masked response to under-flow is
			///   to deliver the denormalized result.
			///   The flush-to-zero mode is provided primarily for performance reasons. At the cost of a slight precision loss, faster
			///   execution can be achieved for applications where underflows
			///   are common and rounding the underflow result to zero can be tolerated. The flush-to-zero bit is cleared upon a
			///   power-up or reset of the processor, disabling the flush-to-zero mode.
			/// </remarks>
			FLUSH_TO_ZERO = 1 << 15,
			...
			/// </summary>
			/// <remarks>
			///   The rounding modes have no effect on comparison operations, operations that produce exact results, or operations that
			///   produce NaN results.
			/// </remarks>
			ROUNDING_CONTROL_MASK = (1 << 14) | (1 << 13),

			/// <summary>
			///   Rounded result is the closest to the infinitely precise result. If two values are equally close, the result is the
			///   even value (that is, the one with the least-significant bit of zero). Default.
			/// </summary>
			ROUND_TO_NEAREST = 0,
			...
			/// </summary>
			ROUND_TOWARD_ZERO = (1 << 13) | (1 << 14),

			/// <summary>
			///   Bits 7 through 12 provide individual mask bits for the SIMD floating-point exceptions. An exception type is
			///   masked if the corresponding mask bit is set, and it is unmasked if the bit is clear. These mask bits are set upon a
			...
			EXCEPTION_MASK =
				...
				| OVERFLOW_MASK
				| DIVIDE_BY_ZERO_MASK
				| DENORMAL_OPERATION_MASK
				| INVALID_OPERATION_MASK,

			/// <summary>
			///   Bit 6 (DAZ) of the MXCSR register enables the denormals-are-zeros mode, which controls the processor’s response to a
			///   SIMD floating-point denormal operand condition.
			/// </summary>
			/// <remarks>
			///   When the denormals-are-zeros flag is set, the processor converts all denormal source operands to a zero with the sign
			///   of the original operand before performing any computations on them.
			///   The processor does not set the denormal-operand exception flag (DE), regardless of the setting of the
			///   denormal-operand exception mask bit (DM); and it does not generate a denormal-operand
			///   exception if the exception is unmasked.The denormals-are-zeros mode is not compatible with IEEE Standard 754.
			///   The denormals-are-zeros mode is provided to improve processor performance for applications such as streaming media
			///   processing, where rounding a denormal operand to zero does not
			///   appreciably affect the quality of the processed data. The denormals-are-zeros flag is cleared upon a power-up or
			///   reset of the processor, disabling the denormals-are-zeros mode.
			///   The denormals-are-zeros mode was introduced in the Pentium 4 and Intel Xeon processor with the SSE2 extensions;
			///   however, it is fully compatible with the SSE SIMD floating-point instructions
			///   (that is, the denormals-are-zeros flag affects the operation of the SSE SIMD floating-point instructions). In earlier
			///   IA-32 processors and in some models of the Pentium 4 processor, this flag
			///   (bit 6) is reserved. Attempting to set bit 6 of the MXCSR register on processors that do not support the DAZ flag
			///   will cause a general-protection exception (#GP).
			/// </remarks>
			DENORMALS_ARE_ZEROES = 1 << 6,

			/// <summary>
			///   Bits 0 through 5 of the MXCSR register indicate whether a SIMD floating-point exception has been detected.
			///   They are "sticky" flags. That is, after a flag is set, it remains set until explicitly cleared. To clear these flags,
			///   use the LDMXCSR or the FXRSTOR instruction to write zeroes to them.
			/// </summary>
			PRECISION_FLAG = 1 << 5,
		...
		{
			readonly MXCSRBits m_OldBits;

			public RoundingScope(MXCSRBits roundingMode)
			{
				m_OldBits = Mxcsr;
				Mxcsr = (m_OldBits & ~MXCSRBits.ROUNDING_CONTROL_MASK) | roundingMode;
			}
...
#if !BURST_INTERNAL
		static void BurstIntrinsicSetCsrFromManaged(int _) { }

		static int BurstIntrinsicGetCsrFromManaged()
		{
			return 0;
		}
		...
		[BurstCompile]
		static void DoSetCsrTrampoline(int bits)
		{
