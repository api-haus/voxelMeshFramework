namespace Voxels.Core.ThirdParty.SurfaceNets.Intrinsics
{
	...
	public static unsafe partial class X86F
	{
		...
		public static class Ssse3
		{
			...
			// _mm_alignr_epi8
			/// <summary>
			///   Concatenate 16-byte blocks in "a" and "b" into a 32-byte temporary result, shift the result right by "count"
			...
			// _mm_hadds_epi16
			/// <summary>
			///   Horizontally add adjacent pairs of 16-bit integers in "a" and "b" using saturation, and pack the signed
			...
			// _mm_hsubs_epi16
			/// <summary>
			///   Horizontally subtract adjacent pairs of 16-bit integers in "a" and "b" using saturation, and pack the signed
			///   16-bit results in "dst".
			/// </summary>
			/// <param name="a">Vector a</param>
			/// <param name="b">Vector b</param>
			...
			// _mm_maddubs_epi16
			/// <summary>
			///   Vertically multiply each unsigned 8-bit integer from "a" with the corresponding signed 8-bit integer from
			///   "b", producing intermediate signed 16-bit integers. Horizontally add adjacent pairs of intermediate signed 16-bit
			...
			public static v128 maddubs_epi16(v128 a, v128 b)
			{
				...
				for (var j = 0; j <= 7; j++)
				{
					var tmp = (aptr[(2 * j) + 1] * bptr[(2 * j) + 1]) + (aptr[2 * j] * bptr[2 * j]);
			...
			// _mm_mulhrs_epi16
			/// <summary>
			///   Multiply packed 16-bit integers in "a" and "b", producing intermediate signed 32-bit integers. Truncate each
			///   intermediate integer to the 18 most significant bits, round by adding 1, and store bits [16:1] to "dst".
			/// </summary>
			/// <param name="a">Vector a</param>
			/// <param name="b">Vector b</param>
			/// <returns>Vector</returns>
